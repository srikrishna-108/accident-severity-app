...
st.markdown(
    "**Smart Road Risk and Severity Analysis System**  —  "
...
if not predict_btn:
    ...
    for col_ui, (title, desc) in zip([c1, c2, c3], tiles):
        with col_ui:
            st.markdown(f"""
...
card_cls, title_cls, _accent = _sev_cfg.get(result, ("sev-noinjury", "clr-noinjury", "#1565c0"))
...
if time_of_day and "night" in time_of_day.lower():
    active_factors.append(("Night-time", "red" if day_type and "weekend" in day_type.lower() else "orange"))
...
def build_reasons():
    ...
    if "over speeding" in c or "speeding" in c:
        if result == "Fatal":
            r.append(("red",
                ...
                "particularly when combined with two-wheelers or a head-on collision type."))
        elif result == "Grievous Injuries":
            r.append(("orange",
            ...
            r.append(("yellow",
                ...
                "and collision type — have limited predicted severity to a lower class in this scenario."))

    if "drunken" in c or "drunk" in c:
        r.append(("red",
            "Drunken Driving impairs reaction time by up to 30% and reduces peripheral vision. "
            "It is one of the strongest single-feature predictors of fatal outcomes in the NH-163 "
            "dataset, independent of road geometry or vehicle type."))
    ...
    if "loss of control" in c:
        r.append(("orange",
            ...
            "or mechanical failure, often leading to secondary collisions or rollovers that escalate "
            "final severity."))

    if "poor visibility" in c or "bad road" in c:
        r.append(("orange",
            "Poor Visibility or Bad Road Condition extends stopping distances and delays hazard "
            "detection. Both conditions are well-established severity amplifiers, particularly at "
            "night and on curved or junction segments."))

    if "junction" in c:
        r.append(("yellow",
            "Junction-related issues such as inadequate sight lines, signal confusion, or conflicting "
            "movements create multiple simultaneous risk interactions, elevating severity beyond what "
            "single-feature analysis would predict."))
    ...
    if accident_type == "Head on":
        r.append(("red",
            ...
            "energy-intensive crash type and a leading cause of fatalities on national highways."))
...
#  Returns list of (title, detail) tuples — no icons
# ─────────────────────────────────────────────────────────────────────────────
def build_recommendations():
    ...
    if "over speeding" in c or "speeding" in c:
        r.append(("Speed enforcement infrastructure",
            ...
            "A 10 km/h reduction in mean speed reduces fatal crashes by approximately 34% (NHAI data)."))

    if "drunken" in c or "drunk" in c:
        r.append(("Impaired driving enforcement",
            "Increase breathalyzer checkpoints during night and weekend windows. "
            "Sustained enforcement rather than one-off campaigns is significantly more effective."))
    ...
    if "poor visibility" in c or "bad road" in c:
        r.append(("Visibility and road surface improvement",
            ...
            "at black spots. Install advance fog warning signs on problem segments."))
    ...
    if offender_vehicle == "Heavy Vehicle":
        r.append(("Heavy vehicle management",
            "Enforce lane discipline for trucks and heavy goods vehicles. Mandate speed limiters "
            "and introduce time-based restrictions near signalised junctions and school zones."))
    ...
    if day_type and "weekend" in day_type.lower():
        r.append(("Weekend traffic management",
            ...
            "message signs for weekend-specific risk campaigns targeting fatigue and impaired driving."))

    # always ensure at least one speed-related recommendation
    if not any("speed" in title.lower() for title, _ in r):
...
with col1:
    ...
    <div class="sev-card {card_cls}">
        <p class="sev-title {title_cls}">{result}</p>
...
# ─────────────────────────────────────────────────────────────────────────────
#  ROW 5  —  Input encoding chart  (all features including Cause)
# ─────────────────────────────────────────────────────────────────────────────
...
with st.expander("Debug — Raw model output"):
    ...
    for i in range(n_classes):
        lbl = severity_labels[i] if i < len(severity_labels) else f"Class {i}"
        pct = int(round(float(probs[i]) * 100))
        st.write(f"- **{lbl}** (class {i}): `{float(probs[i]):.4f}` ({pct}%)")

    st.markdown(
        f"**Predicted class index:** `{pred}` → **`{result}`**  |  "
        f"**Risk score:** `{risk_pct}%` — {risk_label}"
    )
